...
$sql = "SELECT * FROM accepted_bookings";
...
<div class="fixed top-0 left-0 flex h-screen w-64 flex-col bg-white shadow-lg">
  ...
  <div class="p-3 flex flex-col flex-1 overflow-y-auto">
    <a href="admin_board.php" class="flex items-center gap-3 rounded-lg px-3 py-2 mb-2 text-gray-700 hover:bg-gray-200">
    ...
    <div class="relative mb-2">
      ...
      <ul class="dropdown-menu w-full mt-1 border border-gray-200 rounded-lg shadow-sm">
        <li class="bg-green-200"><a class="dropdown-item" href="accepted.php"><i class="fas fa-check-circle me-2 text-green-500"></i>Accepted</a></li>
    ...
    <a href="../logout.php" class="flex items-center gap-3 rounded-lg px-3 py-2 text-red-600 hover:bg-red-100 mt-auto">
      ...
      <div class="ml-64 flex-1 bg-blue-50 p-6 min-h-screen">
        <h1 class="text-3xl font-bold text-gray-800">Accepted</h1>
...
<div class="mt-4">
  ...
  if ($result->num_rows > 0) {
      while($row = $result->fetch_assoc()) {
          ...
          // For small screens, display as stacked cards
          ...
          echo '<div class="sm:table-cell"><span class="font-semibold">Date Accepted:</span> '.date("m/d/Y", strtotime($row["accepted_date"])).'</div>';
          echo '</div>'; // row
          echo '</div>'; // card
      }
  } else {
