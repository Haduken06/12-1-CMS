...
<div class="fixed top-0 left-0 flex h-screen w-64 flex-col bg-white shadow-lg">
  ...
  <div class="p-3 flex flex-col flex-1 overflow-y-auto">
    ...
    <div class="relative mb-2">
      ...
      <ul class="dropdown-menu w-full mt-1 border border-gray-200 rounded-lg shadow-sm">
        ...
        <li class="bg-green-200"><a class="dropdown-item" href="denied.php"><i class="fas fa-ban me-2 text-yellow-500"></i>Denied</a></li>
  ...
  if ($result->num_rows > 0) {
      while($row = $result->fetch_assoc()) {
          ...
          echo '<div class="sm:table-cell"><span class="font-semibold">Date Denied:</span> '.date("m/d/Y", strtotime($row["denied_date"])).'</div>';
          echo '</div>'; // row
          echo '</div>'; // card
      }
  } else {
